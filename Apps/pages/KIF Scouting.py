...
def make_rankings(formation, mins, maximum_mins, data, role_position_df, leagues, exp_contracts, expiration_date,
...
def scout_report(data_frame, gender, league, season, xtra, template, pos, player_pos, pos_in_sample, mins,maximum_mins, minplay, compares, name, ws_name, team, age, sig, extra_text, custom_radar, dist_labels, logo_dict, metric_selections=None):
    plt.clf()
    df = data_frame
    df.replace('3025-06-30','2025-06-30',inplace=True)
...
if gender == 'Men':
    df = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/{full_league_name.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
elif gender == 'Women':
    df = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/Women/{full_league_name.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
df['League'] = full_league_name
...
rank_list = make_rankings(formation, mins,maximum_mins, clean_df, role_position_df, [lg], exp_contracts, expiration_date,
...
with radar_tab:
    ...
    with st.form('Player Radar Options'):
        ...
        dfxxx = df_basic[df_basic['Minutes played'].between(mins,maximum_mins)].copy().reset_index(drop=True)
