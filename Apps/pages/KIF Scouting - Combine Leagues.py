...
st.set_page_config(page_title="KIF Scouting", page_icon="🌍")
...
def calculate_similarity_afl_paper(omega_i, omega_j):
    # Calculate the dot product of omega_i and omega_j
    dot_product = np.dot(omega_i, omega_j)

    # Calculate the magnitudes of omega_i and omega_j
    magnitude_i = np.linalg.norm(omega_i)
    magnitude_j = np.linalg.norm(omega_j)
    ...
    # Calculate the similarity using the given formula
...
def prep_similarity_df(geo_input, region, tiers, time_frame):
    if gender == 'Men':
        sim_lg_lookup = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup.csv')
    if gender == 'Women':
        sim_lg_lookup = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup_women.csv')
    ...
    for _, row in sim_lg_lookup.iterrows():
        ...
        if gender == 'Men':
            url = f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/{formatted_name}.csv'
        if gender == 'Women':
            url = f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/Women/{formatted_name}.csv'
    ...
    full_similarity_df_raw.replace('3025-06-30','2025-06-30',inplace=True)
...
def prep_player_research_table(geo_input, region, tiers, time_frame, mins, maximum_mins, pos_select, min_age, max_age):
    df = prep_similarity_df_filters(geo_input, region, tiers, time_frame)
    return create_player_research_table(df, mins, maximum_mins, pos_select, min_age, max_age)

def similar_players_search(df, ws_id, pos, pca_transform, compare_metrics, mins, maximum_mins, age_band):
    df_base_xx = pd.DataFrame()
    df_filtered_base = filter_by_position_long(df, pos)
    df_filtered_base = df_filtered_base[df_filtered_base['Minutes played'].between(mins,maximum_mins)]
    ...
    extra26 = 'Offensive duels per 90'
    ...
    metrics = {
        ...
        "extra": [
            ...
            extra20, extra21, extra22, extra23, extra24, extra25,extra26
        ]
    }
    ...
    leagues = df_filtered_base.League.unique().tolist()
    for z in range(len(leagues)):
        ...
        df_base_xx = pd.concat([df_base_xx, dfProspect], ignore_index=True)
    
    
    
    df_base_xx = filter_by_position_long(df_base_xx, pos)
    focal_player = df_base_xx.loc[(df_base_xx['Wyscout id'] == ws_id)].iloc[0]
    focal_player_index = df_base_xx[(df_base_xx['Wyscout id'] == ws_id)].index[0]
    ...
    df_compare = df_base_xx[columns_to_compare]
    ...
    for cc in additional_columns:
        similarity_df[cc] = df_base_xx[cc].iloc[similarity_df['Index']].values
    df_compare = similarity_df
    ...
    similar_players = df_compare_sorted[additional_columns + ['Similarity']].rename(columns={'Team within selected timeframe':'Team','Similarity':'Similarity %'})
...
def filter_by_position(df, position, include_in_sample=None):
    if include_in_sample:
        ...
        fbs_wings = ["LB", "RB", "WB","LAMF","RAMF","RW","LW","LWF","RWF"]
        if position == "FB/WBs & Wingers":
            return df[df['Main Position'].str.contains('|'.join(fbs_wings), na=False)]
...
def load_league_data(df, league_season):
    df.replace('3025-06-30','2025-06-30',inplace=True)
...
def make_rankings(generate_all, formation, mins, maximum_mins, data, role_position_df, leagues, exp_contracts, expiration_date,
    ...
    formation_positions = {442:['GK','RCB','LCB','RB','LB','RCM','LCM','RW','LW','RS','LS',],
                            ...
                            343:['GK','RCB','CB','LCB','RB','LB','RCM','LCM','RM','LM','ST'],
    ...
    for q in range(len(rank_11)):
        ...
        for z in range(len(leagues)):
            dfProspect = df[(df['Minutes played'].between(mins,maximum_mins))].copy()
            ...
            extra15 = 'Accurate short / medium passes, %'
            extra16 = 'Passes per 90'
            extra17 = 'Aerial duels won per 90'
            ...
            extra23 = 'Pct of passes being smart'
            extra24 = 'Short / medium passes per 90'
            extra25 = 'Pct of passes being short'
            extra26 = "Passes to final third & deep completions"
            extra27 = "Through passes per 90"
            extra28 = "Head goals per 90"
            extra29 = "Crosses per 90"
            extra30 = "Successful dribbles per 90"
            ...
            ranked_columns_r = [
                ...
                gk2, gk3, gk4, gk5, gk6, gk7, gk8, gk9, gk10, gk12, gk13, gk14,
                ...
                extra22,extra23,extra24,extra25,extra26,extra27,extra28,extra29,extra30
            ]
            inverse_ranked_columns_r = [
                def4,def5,gk1,gk11
            ]
            
            for column in ranked_columns_r:
                dfProspect[f"{column} z"] = stats.zscore(dfProspect[column])
            for column in inverse_ranked_columns_r:
                dfProspect[f"{column} z"] = stats.zscore(dfProspect[column]) * -1
            ...
            for column in ranked_columns_r:
                dfProspect[f"{column} z"] = dfProspect[f"{column} z"] + abs(dfProspect[f"{column} z"].min())
                dfProspect[f"{column} z"] = NormalizeData(dfProspect[f"{column} z"])
    
    
            #############################################################################
            #############################################################################
    
            dfProspect['Distributor Score'] = (
                (.3 * dfProspect['Prevented goals per 90 z']) +
                (.15 * dfProspect['Short / medium passes per 90 z']) +
                (.15 * dfProspect['Pct of passes being short z']) +
                (.15 * dfProspect['Accurate long passes, % z']) +
                (.2 * dfProspect['Progressive passes per 90 z']) +
                (.05 * dfProspect['Save rate, % z'])
            )
            dfProspect['Pure Shot-Stopper Score'] = (
                (.5 * dfProspect['Prevented goals per 90 z']) +
                (.1 * dfProspect['Short / medium passes per 90 z']) +
                (.05 * dfProspect['Pct of passes being short z']) +
                (.1 * dfProspect['Accurate long passes, % z']) +
                (.15 * dfProspect['Save rate, % z']) +
                (.1 * dfProspect['Exits per 90 z']) 
            )
            dfProspect['Balanced Keeper Score'] = (
                (.4 * dfProspect['Prevented goals per 90 z']) +
                (.15 * dfProspect['Short / medium passes per 90 z']) +
                (.1 * dfProspect['Pct of passes being short z']) +
                (.1 * dfProspect['Accurate long passes, % z']) +
                (.1 * dfProspect['Progressive passes per 90 z']) +
                (.1 * dfProspect['Save rate, % z']) +
                (.05 * dfProspect['Exits per 90 z']) 
            )
            dfProspect['Ball Playing CB Score'] = (
                (.15 * dfProspect['Accurate long passes, % z']) +
                (.2 * dfProspect['Passes to final third & deep completions z']) +
                (.2 * dfProspect['Progressive passes per 90 z']) +
                (.1 * dfProspect['Aerial duels won, % z']) +
                (.1 * dfProspect['Defensive duels won, % z']) +
                (.1 * dfProspect['Aerial duels won per 90 z']) +
                (.1 * dfProspect['Through passes per 90 z']) +
                (.05 * dfProspect['Progressive runs per 90 z'])
            )
            dfProspect['Wide CB Score'] = (
                (.1 * dfProspect['Passes to final third & deep completions z']) +
                (.1 * dfProspect['Progressive passes per 90 z']) +
                (.1 * dfProspect['Aerial duels won, % z']) +
                (.15 * dfProspect['Defensive duels won, % z']) +
                (.1 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) +
                (.25 * dfProspect['Progressive runs per 90 z']) + 
                (.15 * dfProspect['Crosses per 90 z'])
            )
            dfProspect['Defensive CB Score'] = (
                (.05 * dfProspect['Accurate long passes, % z']) +
                (.1 * dfProspect['Passes to final third & deep completions z']) +
                (.1 * dfProspect['Progressive passes per 90 z']) +
                (.25 * dfProspect['Aerial duels won, % z']) +
                (.2 * dfProspect['Defensive duels won, % z']) +
                (.25 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z'])
            )
            dfProspect['Goalscoring CB Score'] = (
                (.05 * dfProspect['Accurate long passes, % z']) +
                (.05 * dfProspect['Passes to final third & deep completions z']) +
                (.05 * dfProspect['Progressive passes per 90 z']) +
                (.15 * dfProspect['Aerial duels won, % z']) +
                (.1 * dfProspect['Defensive duels won, % z']) +
                (.15 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) + 
                (.1 * dfProspect['Shots per 90 z']) + 
                (.15 * dfProspect['Touches in box per 90 z']) + 
                (.1 * dfProspect['npxG per 90 z']) + 
                (.05 * dfProspect['Head goals per 90 z'])
            )
            dfProspect['Balanced CB Score'] = (
                (.05 * dfProspect['Accurate long passes, % z']) +
                (.15 * dfProspect['Passes to final third & deep completions z']) +
                (.15 * dfProspect['Progressive passes per 90 z']) +
                (.2 * dfProspect['Aerial duels won, % z']) +
                (.15 * dfProspect['Defensive duels won, % z']) +
                (.2 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) +
                (.05 * dfProspect['Progressive runs per 90 z'])
            )
            dfProspect['Defensive FB Score'] = (
                (.05 * dfProspect['Passes to final third & deep completions z']) +
                (.075 * dfProspect['Progressive passes per 90 z']) +
                (.15 * dfProspect['Aerial duels won, % z']) +
                (.2 * dfProspect['Defensive duels won, % z']) +
                (.175 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) +
                (.05 * dfProspect['PAdj Sliding tackles z']) +
                (.1 * dfProspect['PAdj Interceptions z']) +
                (.05 * dfProspect['Crosses per 90 z']) +
                (.05 * dfProspect['Passes to penalty area per 90 z']) +
                (.05 * dfProspect['Defensive duels per 90 z'])
            )
            dfProspect['Inverted FB Score'] = (
                (.1 * dfProspect['Passes to final third & deep completions z']) +
                (.15 * dfProspect['Progressive passes per 90 z']) +
                (.05 * dfProspect['Aerial duels won, % z']) +
                (.1 * dfProspect['Defensive duels won, % z']) +
                (.1 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) +
                (.05 * dfProspect['PAdj Sliding tackles z']) +
                (.1 * dfProspect['PAdj Interceptions z']) +
                (.1 * dfProspect['Crosses per 90 z']) +
                (.05 * dfProspect['Passes to penalty area per 90 z']) +
                (.1 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Shot assists per 90 z'])
            )
            dfProspect['Attacking FB Score'] = (
                (.05 * dfProspect['Passes to final third & deep completions z']) +
                (.05 * dfProspect['Aerial duels won, % z']) +
                (.05 * dfProspect['Defensive duels won, % z']) +
                (.05 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Shots per 90 z']) +
                (.05 * dfProspect['Touches in box per 90 z']) +
                (.075 * dfProspect['PAdj Interceptions z']) +
                (.2 * dfProspect['Crosses per 90 z']) +
                (.05 * dfProspect['Passes to penalty area per 90 z']) +
                (.075 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Shot assists per 90 z']) +
                (.05 * dfProspect['xA per 90 z']) +
                (.1 * dfProspect['Successful dribbles per 90 z']) +
                (.1 * dfProspect['Progressive runs per 90 z'])
            )
            dfProspect['Wingback Score'] = (
                (.05 * dfProspect['Defensive duels won, % z']) +
                (.05 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Shots per 90 z']) +
                (.05 * dfProspect['Touches in box per 90 z']) +
                ...
                (.05 * dfProspect['Passes to penalty area per 90 z']) +
                (.1 * dfProspect['Defensive duels per 90 z']) +
                (.075 * dfProspect['Shot assists per 90 z']) +
                (.075 * dfProspect['xA per 90 z']) +
                (.2 * dfProspect['Successful dribbles per 90 z']) +
                (.05 * dfProspect['npxG per 90 z']) +
                (.1 * dfProspect['Progressive runs per 90 z'])
            )
            dfProspect['Midfield Anchor Score'] = (
                (.1 * dfProspect['Defensive duels won, % z']) +
                (.3 * dfProspect['Defensive duels per 90 z']) +
                (.1 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Aerial duels won, % z']) +
                (.15 * dfProspect['PAdj Sliding tackles z']) +
                (.15 * dfProspect['PAdj Interceptions z']) +
                (.05 * dfProspect['Progressive passes per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) +
                (.05 * dfProspect['Passes to final third per 90 z'])
            )
            dfProspect['Holding Mid Score'] = (
                (.075 * dfProspect['Defensive duels won, % z']) +
                (.225 * dfProspect['Defensive duels per 90 z']) +
                (.075 * dfProspect['Aerial duels won per 90 z']) +
                (.03 * dfProspect['Aerial duels won, % z']) +
                (.1 * dfProspect['PAdj Sliding tackles z']) +
                (.1 * dfProspect['PAdj Interceptions z']) +
                (.15 * dfProspect['Progressive passes per 90 z']) +
                (.08 * dfProspect['Through passes per 90 z']) +
                (.1 * dfProspect['Passes to final third per 90 z']) +
                (.065 * dfProspect['Passes to penalty area per 90 z'])
            )
            dfProspect['Deep-Lying Playmaker Score'] = (
                (.05 * dfProspect['Defensive duels won, % z']) +
                (.15 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['PAdj Sliding tackles z']) +
                (.05 * dfProspect['PAdj Interceptions z']) +
                (.2 * dfProspect['Progressive passes per 90 z']) +
                (.1 * dfProspect['Through passes per 90 z']) +
                (.15 * dfProspect['Passes to final third per 90 z']) +
                (.1 * dfProspect['Passes to penalty area per 90 z']) +
                (.05 * dfProspect['Smart passes per 90 z']) +
                (.05 * dfProspect['xA per 90 z'])
            )
            dfProspect['Box-to-Box Midfielder Score'] = (
                (.05 * dfProspect['Defensive duels won, % z']) +
                (.2 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['PAdj Sliding tackles z']) +
                (.05 * dfProspect['PAdj Interceptions z']) +
                (.1 * dfProspect['Progressive passes per 90 z']) +
                (.06 * dfProspect['Through passes per 90 z']) +
                (.06 * dfProspect['Passes to final third per 90 z']) +
                (.06 * dfProspect['Passes to penalty area per 90 z']) +
                (.05 * dfProspect['Smart passes per 90 z']) +
                (.1 * dfProspect['Touches in box per 90 z']) +
                (.12 * dfProspect['Progressive runs per 90 z']) +
                (.05 * dfProspect['Shots per 90 z'])
            )
            dfProspect['Advanced Playmaker Score'] = (
                (.15 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['PAdj Interceptions z']) +
                (.06 * dfProspect['Progressive passes per 90 z']) +
                (.08 * dfProspect['Through passes per 90 z']) +
                (.06 * dfProspect['Passes to final third per 90 z']) +
                (.08 * dfProspect['Passes to penalty area per 90 z']) +
                (.08 * dfProspect['Smart passes per 90 z']) +
                (.06 * dfProspect['Touches in box per 90 z']) +
                (.06 * dfProspect['Successful dribbles per 90 z']) +
                (.06 * dfProspect['Shots per 90 z']) +
                (.06 * dfProspect['npxG per 90 z']) +
                (.08 * dfProspect['xA per 90 z']) +
                (.08 * dfProspect['Shot assists per 90 z']) +
                (.04 * dfProspect['Crosses per 90 z'])
            )
            dfProspect['Classic 10 Score'] = (
                (.15 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['PAdj Interceptions z']) +
                (.08 * dfProspect['Through passes per 90 z']) +
                (.08 * dfProspect['Passes to penalty area per 90 z']) +
                (.05 * dfProspect['Smart passes per 90 z']) +
                (.08 * dfProspect['Touches in box per 90 z']) +
                (.15 * dfProspect['Successful dribbles per 90 z']) +
                (.08 * dfProspect['Shots per 90 z']) +
                (.08 * dfProspect['npxG per 90 z']) +
                (.1 * dfProspect['xA per 90 z']) +
                (.1 * dfProspect['Shot assists per 90 z'])
            )
            dfProspect['Second Striker Score'] = (
                (.15 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Through passes per 90 z']) +
                (.25 * dfProspect['Touches in box per 90 z']) +
                (.07 * dfProspect['Successful dribbles per 90 z']) +
                (.13 * dfProspect['Shots per 90 z']) +
                (.23 * dfProspect['npxG per 90 z']) +
                (.06 * dfProspect['xA per 90 z']) +
                (.06 * dfProspect['Shot assists per 90 z'])
            )
            dfProspect['Traditional Winger Score'] = (
                (.1 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Touches in box per 90 z']) +
                (.4 * dfProspect['Successful dribbles per 90 z']) +
                (.17 * dfProspect['Crosses per 90 z']) +
                (.04 * dfProspect['Shots per 90 z']) +
                (.04 * dfProspect['npxG per 90 z']) +
                (.1 * dfProspect['xA per 90 z']) +
                (.1 * dfProspect['Shot assists per 90 z'])
            )
            dfProspect['Inverted Winger Score'] = (
                (.125 * dfProspect['Defensive duels per 90 z']) +
                (.075 * dfProspect['Passes to penalty area per 90 z']) +
                (.125 * dfProspect['Touches in box per 90 z']) +
                (.2 * dfProspect['Successful dribbles per 90 z']) +
                (.05 * dfProspect['Crosses per 90 z']) +
                (.12 * dfProspect['Shots per 90 z']) +
                (.12 * dfProspect['npxG per 90 z']) +
                (.1 * dfProspect['xA per 90 z']) +
                (.085 * dfProspect['Shot assists per 90 z'])
            )
            dfProspect['Inside Forward Score'] = (
                (.125 * dfProspect['Defensive duels per 90 z']) +
                (.12 * dfProspect['Touches in box per 90 z']) +
                (.125 * dfProspect['Successful dribbles per 90 z']) +
                (.05 * dfProspect['Crosses per 90 z']) +
                (.225 * dfProspect['Shots per 90 z']) +
                (.2 * dfProspect['npxG per 90 z']) +
                (.08 * dfProspect['xA per 90 z']) +
                (.075 * dfProspect['Shot assists per 90 z'])
            )
            dfProspect['Wide Playmaker Score'] = (
                (.1 * dfProspect['Defensive duels per 90 z']) +
                (.05 * dfProspect['Touches in box per 90 z']) +
                (.1 * dfProspect['Successful dribbles per 90 z']) +
                (.05 * dfProspect['npxG per 90 z']) +
                (.15 * dfProspect['xA per 90 z']) +
                (.20 * dfProspect['Shot assists per 90 z']) +
                (.1 * dfProspect['Passes to penalty area per 90 z']) +
                (.1 * dfProspect['Through passes per 90 z']) +
                (.15 * dfProspect['Progressive passes per 90 z'])
            )
            dfProspect['Complete Striker Score'] = (
                (.1 * dfProspect['Defensive duels per 90 z']) +
                (.15 * dfProspect['Touches in box per 90 z']) +
                (.1 * dfProspect['Successful dribbles per 90 z']) +
                (.175 * dfProspect['Shots per 90 z']) +
                (.175 * dfProspect['npxG per 90 z']) +
                (.075 * dfProspect['xA per 90 z']) +
                (.075 * dfProspect['Shot assists per 90 z']) +
                (.075 * dfProspect['Aerial duels won per 90 z']) +
                (.075 * dfProspect['Aerial duels won, % z'])
            )
            dfProspect['Target Man Score'] = (
                (.075 * dfProspect['Defensive duels per 90 z']) +
                (.15 * dfProspect['Touches in box per 90 z']) +
                (.125 * dfProspect['Shots per 90 z']) +
                (.175 * dfProspect['npxG per 90 z']) +
                (.175 * dfProspect['Aerial duels won per 90 z']) +
                (.15 * dfProspect['Aerial duels won, % z']) +
                (.15 * dfProspect['Head goals per 90 z'])
            )
            dfProspect['Poacher Score'] = (
                (.075 * dfProspect['Defensive duels per 90 z']) +
                (.2 * dfProspect['Touches in box per 90 z']) +
                (.075 * dfProspect['Successful dribbles per 90 z']) +
                (.2 * dfProspect['Shots per 90 z']) +
                (.25 * dfProspect['npxG per 90 z']) +
                (.05 * dfProspect['xA per 90 z']) +
                (.05 * dfProspect['Shot assists per 90 z']) +
                (.05 * dfProspect['Aerial duels won per 90 z']) +
                (.05 * dfProspect['Aerial duels won, % z'])
            ...
            dfProspect['Playmaking Striker Score'] = (
                ...
                (.1 * dfProspect['Shots per 90 z']) +
                (.1 * dfProspect['npxG per 90 z']) +
                (.125 * dfProspect['xA per 90 z']) +
                (.175 * dfProspect['Shot assists per 90 z']) +
                (.095 * dfProspect['Passes to penalty area per 90 z']) +
                (.11 * dfProspect['Through passes per 90 z']) +
                (.07 * dfProspect['Progressive passes per 90 z'])
            )
            dfProspect['Transitional Threat Score'] = (
                (.1 * dfProspect['Defensive duels per 90 z']) +
                (.1 * dfProspect['Touches in box per 90 z']) +
                (.2 * dfProspect['Successful dribbles per 90 z']) +
                (.15 * dfProspect['Shots per 90 z']) +
                (.2 * dfProspect['npxG per 90 z']) +
                (.05 * dfProspect['xA per 90 z']) +
                (.05 * dfProspect['Aerial duels won per 90 z']) +
                (.15 * dfProspect['Progressive runs per 90 z'])
            )
            dfProspect['Pressing Striker Score'] = (
                (.3 * dfProspect['Defensive duels per 90 z']) +
                (.1 * dfProspect['PAdj Interceptions z']) +
                (.1 * dfProspect['Touches in box per 90 z']) +
                (.075 * dfProspect['Successful dribbles per 90 z']) +
...
def scout_report(data_frame, gender, league, season, full_league_name, xtra, template, pos, player_pos, pos_in_sample, mins,maximum_mins, minplay, compares, name, ws_name, team, age, sig, extra_text, custom_radar, dist_labels, logo_dict, com_lg_txt, metric_selections=None):
    plt.clf()
    ...
    # Filter data
    dfProspect = df[(df['Minutes played'].between(mins,maximum_mins))].copy()
    fallback_raw_valsdf = dfProspect[(dfProspect['Player']==ws_name) & (dfProspect['Team within selected timeframe']==team) & (dfProspect['Age']==age) & (dfProspect['League']==full_league_name)]
    dfProspect = filter_by_position(dfProspect, pos, include_in_sample=pos_in_sample)
    raw_valsdf = dfProspect[(dfProspect['Player']==ws_name) & (dfProspect['Team within selected timeframe']==team) & (dfProspect['Age']==age) & (dfProspect['League']==full_league_name)]
    if len(raw_valsdf)==0:
    ...
    dfRadarMF = dfProspect[(dfProspect['Player']==ws_name) & (dfProspect['Team within selected timeframe']==team) & (dfProspect['Age']==age) & (dfProspect['League']==full_league_name)].reset_index(drop=True)
    ...
    if dist_labels == 'No':
        dist_text = ""
    plt.annotate(f"Bars are percentiles | Values shown are {callout_text} values\nAll values are per 90 minutes | %s\nCompared to {com_lg_txt} %s %s, %i-%i mins\nData: Wyscout | Sample Size: %i players{dist_text}" %(extra_text, league, compares, mins,maximum_mins, len(dfProspect)),
...
def create_player_research_table(df_basic, mins, maximum_mins, pos, min_age, max_age):
    dfProspect = df_basic[(df_basic['Minutes played'].between(mins,maximum_mins))].copy()
    ...
    extra12 = 'Offensive duels per 90'
    extra13 = 'Offensive duels won, %'

    dfProspect['Offensive duels per 90'].fillna(0,inplace=True)
    dfProspect['Offensive duels won, %'].fillna(0,inplace=True)
    dfProspect['Offensive duels per 90'] = dfProspect['Offensive duels per 90'].astype(float)
    dfProspect['Offensive duels won, %'] = dfProspect['Offensive duels won, %'].astype(float)
    ...
    'fwdpct1','fwdpct2','fwdpct5','fwdpct6','fwdpct11','midpct1','midpct3','midpct4','midpct5','midpct6','midpct7','midpct8','midpct9','midpct10','midpct11','midpct12','defpct1','defpct2','defpct3','defpct4','defpct5','defpct6','defpct7','defpct8','defpct9','defpct10',
                        ...
                        'extrapct','extrapct2','extrapct3','extrapct4','extrapct5','extrapct6','extrapct7','extrapct8','extrapct9','extrapct10','extrapct11','extrapct12','extrapct13'
    ]]
    ...
    'extrapct11': 'Successful dribbles per 90',
      'extrapct12':'Offensive duels per 90',
      'extrapct13':'Offensive duels won, %',
...
rank_11_base = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/Ranking_XI_KIF.csv')
role_position_lookup = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/Role_Positions_Lookup_KIF.csv')
...
ws_pos_compare_groups = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/WyscoutPositionsComparisons.csv')

with st.sidebar:
    st.header('Choose Gender')
    gender = st.selectbox('Gender', ('Men','Women'))
if gender == 'Men':
    lg_lookup = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup.csv')
    default_danish_league = "Danish 1. Division"
    default_danish_season = "25-26"
    logo_dict_df = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/Wyscout_Logo_Dict.csv')
    logo_dict = pd.Series(logo_dict_df['Team logo'].values,index=logo_dict_df['Team']).to_dict()

if gender == 'Women':
    lg_lookup = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup_women.csv')
    default_danish_league = "Danish A-Liga"
    default_danish_season = "25-26"
    logo_dict_df = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/Wyscout_Logo_Dict_Women.csv')
    logo_dict = pd.Series(logo_dict_df['Team logo'].values,index=logo_dict_df['Team']).to_dict()
...
with st.sidebar:
    lg = st.selectbox('League', options=legaues, index=legaues.index(default_danish_league))
...
with st.sidebar:
    ...
    maximum_mins = st.number_input('Maximum Minutes Played', 500, 10000, 5000)
    ...
    if exp_contracts_ == 'Yes':
        exp_contracts = 'y'
        expiration_date = st.date_input("Contract Expires On Or Before", datetime.date(2026, 8, 1), format='YYYY-MM-DD')
    else:
        exp_contracts = 'n'
    ...
    with st.form('Role-Positions'):
        ...
        pos9 = st.selectbox(formation_positions[formation][8], (role_position_lookup[role_position_lookup.form_pos == formation_positions[formation][8]].pos_role.tolist()), index=2)
...
leagues_xx = lg_lookup[(lg_lookup.League==lg)].League.tolist()
seasons_xx = lg_lookup[(lg_lookup.League==lg)].Season.tolist()

    
if gender == 'Men':
    foc_ssn_df = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/{full_league_name.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
    df = pd.DataFrame()
    for l, s in zip(leagues_xx,seasons_xx):
        full_league_name_xx = f"{l} {s}"
        dd_dd = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/{full_league_name_xx.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
        dd_dd['League'] = full_league_name_xx
        df = pd.concat([df,dd_dd],ignore_index=True)
        del dd_dd
elif gender == 'Women':
    foc_ssn_df = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/Women/{full_league_name.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
    df = pd.DataFrame()
    for l, s in zip(leagues_xx,seasons_xx):
        full_league_name_xx = f"{l} {s}"
        dd_dd = pd.read_csv(f'https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/refs/heads/main/Main%20App/Women/{full_league_name_xx.replace(" ","%20").replace("ü","u").replace("ó","o").replace("ö","o").replace("ã","a").replace("ç","c").replace("é","e")}.csv')
        dd_dd['League'] = full_league_name_xx
        df = pd.concat([df,dd_dd],ignore_index=True)
        del dd_dd
...
clean_df = load_league_data(df, full_league_name)
foc_clean_df = clean_df[clean_df.League==full_league_name].reset_index(drop=True)
df_basic = clean_df.copy()

with st.sidebar:
    one_team_choice = st.selectbox('One Team Depth Chart?', (['No','Yes']))
    if one_team_choice == 'Yes':
        chosen_team = st.selectbox('Team', (sorted(foc_clean_df['Team within selected timeframe'].unique().tolist())))
...
rank_list = make_rankings(False,formation, mins,maximum_mins, foc_clean_df, role_position_df, [lg], exp_contracts, expiration_date,
...
all_total_rank_list = make_rankings(True,formation, mins,maximum_mins, foc_clean_df, role_position_df, [lg], exp_contracts, expiration_date,
                                    min_age=ages[0], max_age=ages[1], num=number_of_players, normalize_to_100=normalize_to_100, chosen_team=chosen_team, nationality_chosen=chosen_nations)
...
def make_fig(ages,exp_contracts,rank_11_base,show_ranks2,season,lg,normalize_to_100,team_text,chosen_nations):
    ...
    min_text = f"Between {mins} & {maximum_mins}"
    
    if ages[0] == 0 and ages[1] == 45:
        age_text = f'{min_text} minutes played'
    elif ages[0] == 0:
        age_text = f'{min_text} minutes played | U{ages[1]} players only'
    elif ages[1] == 45:
        age_text = f'{min_text} minutes played | Players {ages[0]} & older'
    else:
        age_text = f'{min_text} minutes played | Players between {ages[0]} & {ages[1]}'
...
image_tab, table_tab, radar_tab, all_player_list_tab, full_ranks_tab, similarity_tab, filter_tab, filter_table_tab, scatter_tab,ranking_tab,  notes_tab, map_tab = st.tabs(['Role Ranking Image', 'Role Ranking Table', 'Player Radar Generation', 'Player List', 'Full League Ranks', 'Similar Player Search', 'Player Search, Filters', 'Player Search, Results', 'Scatter Plots', 'Custom Player Ranking Model', 'Role Score Definitions & Calculations', 'Available Leagues'])

with full_ranks_tab:
    full_ranks_list = all_total_rank_list
    score_range = st.slider('Role Score', 0, 100, (0, 100))
    single_team = st.multiselect('Team(s)', (sorted(foc_clean_df['Team within selected timeframe'].unique().tolist())))
    one_role = st.multiselect('Role(s)', (sorted(all_total_rank_list['Squad Position'].unique().tolist())))
    one_player = st.multiselect('Player(s)', (sorted(all_total_rank_list['Player'].unique().tolist())))


    if single_team:
        full_ranks_list = full_ranks_list[full_ranks_list['Team'].isin(single_team)]
    if one_role:
        full_ranks_list = full_ranks_list[full_ranks_list['Squad Position'].isin(one_role)]
    if one_player:
        full_ranks_list = full_ranks_list[full_ranks_list['Player'].isin(one_player)]

    full_ranks_list = full_ranks_list[full_ranks_list['Score'].between(score_range[0], score_range[1])]

    full_ranks_list = full_ranks_list[['Role Rank','Score','Player','Team','Age','Player Pos.','Squad Position','Minutes played','Contract expires']].sort_values(by='Score',ascending=False).reset_index(drop=True).rename(columns={'Squad Position':'Role'})
...
with radar_tab:
    ...
    with st.form('Player Radar Options'):
        ...
        all_comps = st.selectbox(f'Compare to All Seasons or just {season}?', ('All',season))
        submitted = st.form_submit_button("Submit Options")
        ...
        if all_comps == 'All':
            ...
            df_to_pass = foc_clean_df
            com_lg_txt = f"{season}"

        dfxxx = df_to_pass[df_to_pass['Minutes played'].between(mins,maximum_mins)].reset_index(drop=True)
        # dfxxx = dfxxx[dfxxx['League']==full_league_name].reset_index(drop=True)
        df1 = dfxxx[['Wyscout id', 'Player', 'Team within selected timeframe', 'Position', 'Age', 'Minutes played','League']]
        ...
        try:
            gen = df1[(df1['Player']==player) & (df1['Age']==page) & (df1['League']==full_league_name)]
...
with all_player_list_tab:
    if chosen_team == 'N/A':
        player_list_df = foc_clean_df.copy()
        player_list_df = player_list_df.sort_values(by=['Player']).reset_index(drop=True)
        player_list_df = player_list_df[['Wyscout id','Full name','Player','Age','Team within selected timeframe','Position','Minutes played','On loan','Passport country','League']].rename(columns={'Player':'Radar Name','Team within selected timeframe':'Team','Position':'Positions'})
        player_list_df
    if chosen_team != 'N/A':
        player_list_df = foc_clean_df.copy()
...
with similarity_tab:
    with st.form('Similar Player Search'):
        ...
        if gender == 'Men':
            similar_player_lg_lookup = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup.csv')
        if gender == 'Women':
            similar_player_lg_lookup = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup_women.csv')
        geo_input = st.selectbox("Geography Region", ('League','Country',"Region",'Continent','Scouting Group'))
        geo_mapping = {
            'Region': 'Nordics',
            'Country': 'Denmark',
            'Continent': 'Europe',
            'League': default_danish_league,
        ...
        sim_pos = st.selectbox('Positions', ('Strikers', 'Strikers and Wingers', 'Forwards (AM, W, CF)',
                                ...
                                'Central Midfielders no CAM (DM, CM)', 'Central Midfielders no DM (CM, CAM)', 'Fullbacks (FBs/WBs)','FB/WBs & Wingers',
                                'Defenders (CB, FB/WB, DM)', 'Centre-Backs', 'CBs & DMs','Goalkeepers'))
        pca_transform = st.selectbox('Dimensionality Reduction?', ('No','Yes'))
        custom_metric_comparison = st.selectbox('Pre-Made or Custom Metrics to Compare By?', ('Custom','Pre-Made'))
        if custom_metric_comparison == 'Custom':
            compare_metrics = st.multiselect(label="Select the metrics you want on the radar", options=['Received passes per 90','Passes per 90','Pct of passes being short','Pct of passes being lateral','Accurate passes, %','Accurate short / medium passes, %','Accurate long passes, %','Crosses per 90','Accurate crosses, %','Smart passes per 90','Shot assists per 90','xA per 90','xA per Shot Assist','Assists per 90','Second assists per 90','Third assists per 90','1st, 2nd, 3rd assists','Progressive passes per 90','Progressive runs per 90','Shots per 90','npxG per 90','Non-penalty goals per 90','npxG per shot','Goal conversion, %','Successful dribbles, %','Accelerations per 90','Touches in box per 90','Fouls suffered per 90','Defensive duels per 90','Successful defensive actions per 90','Duels won, %','Defensive duels won, %','pAdj Tkl+Int per 90','PAdj Sliding tackles','PAdj Interceptions','Shots blocked per 90','Aerial duels per 90','Aerial duels won, %','Aerial duels won per 90','Fouls per 90','Shots against per 90','Conceded goals per 90','Save rate, %','Prevented goals per 90','Goals prevented %','Clean sheets, %','Exits per 90','Average long pass length, m','Accurate progressive passes, %','Deep completed crosses per 90','Passes to penalty area per 90','Passes to final third per 90','Pct of passes being smart','Key passes per 90','Through passes per 90','Dribbles per 90'], default=['Passes per 90','npxG per 90','Shot assists per 90','Defensive duels per 90',])
...
with filter_tab:
    with st.form('Position Filter Min Filters'):
        ...
        pos_select_filters = st.selectbox('Positions', ('Strikers', 'Strikers and Wingers', 'Forwards (AM, W, CF)',
                                ...
                                'Defenders (CB, FB/WB, DM)', 'Centre-Backs', 'CBs & DMs','Goalkeepers'))
        if gender == 'Men':
            similar_player_lg_lookup_filters = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup.csv')
        if gender == 'Women':
            similar_player_lg_lookup_filters = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup_women.csv')
    ...
    if geo_input_filters == 'League':
        with st.form('Time Frame Filters, League'):
            ...
            time_frame_filters = st.multiselect('League-Seasons', (similar_player_lg_lookup_filters[similar_player_lg_lookup_filters.League.isin(region_filters)].sort_values(by=['Country','Tier','Season'],ascending=[True,True,False])['League-Season'].tolist()), default=f"{default_danish_league} {default_danish_season}")
    ...
    try:
        ...
        region_filters=default_danish_league,
        tiers_filters=[],
        time_frame_filters=f"{default_danish_league} {default_danish_season}"
        pos_select_filters = 'Strikers'
    raw_df_for_filtering = prep_player_research_table(geo_input_filters, region_filters, tiers_filters, time_frame_filters, mins, maximum_mins, pos_select_filters, ages[0], ages[1])
    numeric_columns = raw_df_for_filtering.select_dtypes(include='number')
    min_dict = numeric_columns.min()
    max_dict = numeric_columns.max()
    #########
...
with filter_tab:
    ...
    with st.form('Minimum Percentile Filters'):
        ...
        if ['slider1','slider2','slider3','slider4','slider5','slider6','slider7','slider8','slider9','slider10','slider11','slider12','slider13','slider14','slider15','slider16','slider17','slider18','slider19','slider20','slider21','slider22','slider23','slider24','slider25','slider26','slider27','slider28','slider29','slider30','slider31','slider32','slider33','slider34','slider35','slider36','slider37','slider38','slider39','slider40','slider41','slider42'] not in st.session_state:
            pass
        ...
        num_dribs = st.slider('Successful dribbles per 90', min_dict['Successful dribbles per 90'], max_dict['Successful dribbles per 90'], key='slider20')
        off_duels = st.slider('Offensive duels per 90', min_dict['Offensive duels per 90'], max_dict['Offensive duels per 90'], key='slider21')
        off_win_pct = st.slider('Offensive Duel Win %', min_dict['Offensive duels won, %'], max_dict['Offensive duels won, %'], key='slider22')
        accel = st.slider('Accelerations per 90', min_dict['Accelerations per 90'], max_dict['Accelerations per 90'], key='slider23')
        progcarry = st.slider('Progressive Carries per 90', min_dict['Progressive runs per 90'], max_dict['Progressive runs per 90'], key='slider24')
        progpass = st.slider('Progressive Passes per 90', min_dict['Progressive passes per 90'], max_dict['Progressive passes per 90'], key='slider25')
        aerial = st.slider('Aerial Win %', min_dict['Aerial duels won, %'], max_dict['Aerial duels won, %'], key='slider26')
        aerialswon = st.slider('Aerials Won per 90', min_dict['Aerial duels won per 90'], max_dict['Aerial duels won per 90'], key='slider27')
        defduels = st.slider('Defensive Duels Success %', min_dict['Defensive duels won, %'], max_dict['Defensive duels won, %'], key='slider28')
        defend = st.slider('Successful Defensive Actions per 90', min_dict['Successful defensive actions per 90'], max_dict['Successful defensive actions per 90'], key='slider29')
        tklint = st.slider('Tackles & Interceptions per 90', min_dict['pAdj Tkl+Int per 90'], max_dict['pAdj Tkl+Int per 90'], key='slider30')
        tkl = st.slider('Sliding Tackles per 90', min_dict['PAdj Sliding tackles'], max_dict['PAdj Sliding tackles'], key='slider31')
        intercept = st.slider('Interceptions per 90', min_dict['PAdj Interceptions'], max_dict['PAdj Interceptions'], key='slider32')
        shotblock = st.slider('Shots Blocked per 90', min_dict['Shots blocked per 90'], max_dict['Shots blocked per 90'], key='slider33')
        foul = st.slider('Fouls Committed per 90', min_dict['Fouls per 90'], max_dict['Fouls per 90'], key='slider34')
        fouldraw = st.slider('Fouls Drawn per 90', min_dict['Fouls suffered per 90'], max_dict['Fouls suffered per 90'], key='slider35')
        cards = st.slider('Cards per 90', min_dict['Cards per 90'], max_dict['Cards per 90'], key='slider36')
        if pos_select_filters == 'Goalkeepers':
            concede = st.slider('Conceded goals per 90', min_dict['Conceded goals per 90'], max_dict['Conceded goals per 90'], key='slider37')
            saverate = st.slider('Save rate, %', min_dict['Save rate, %'], max_dict['Save rate, %'], key='slider38')
            exits = st.slider('Exits per 90', min_dict['Exits per 90'], max_dict['Exits per 90'], key='slider39')
            psxg = st.slider('Prevented goals per 90', min_dict['Prevented goals per 90'], max_dict['Prevented goals per 90'], key='slider40')
            gppct = st.slider('Goals prevented %', min_dict['Goals prevented %'], max_dict['Goals prevented %'], key='slider41')
            shotsfaced = st.slider('Shots against per 90', min_dict['Shots against per 90'], max_dict['Shots against per 90'], key='slider42')
...
with filter_table_tab:
    ...
    player_research_table = final[(final['Accurate short / medium passes, %']>=short) &
                 ...
                 (final['Passes per 90']>=passestot) &
                  ...
                  (final['Offensive duels won, %']>=off_win_pct) &
                  (final['Offensive duels per 90']>=off_duels) &
...
with scatter_tab:
    ...
    with st.form("Scatter Options"):
        ...
        dfProspect_scatter = scatter_df[(scatter_df['Minutes played'].between(mins,maximum_mins)) & (scatter_df['League'] == full_league_name)].copy()
    ...
    fig_scatter = px.scatter(
        ...
        hover_name = 'Player',
        title = '%s %s, %s & %s <br><sup>%s%s %i-%i minutes played | %s</sup>' %(season,lg,xx,yy,age_text_scatter,pos_select_scatter,mins,maximum_mins,update_date),
        width=900,
        height=700)
...
with ranking_tab:
    with st.form('Geo Selection for Ranking'):
        ...
        if gender == 'Men':
            similar_player_lg_lookup_ranking = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup.csv')
        if gender == 'Women':
            similar_player_lg_lookup_ranking = pd.read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup_women.csv')
    ...
    try:
        ...
        tiers_rankings=[],
        time_frame_ranking=f'{default_danish_league} {default_danish_season}'
    ...
    ranking_df = ranking_df[ranking_df['Minutes played'].between(mins,maximum_mins)]
    ...
    if metrics:
        ...
        rank_df_final_choice = df_filtered.sort_values("Score", ascending=False)[["Wyscout id","Player","Team within selected timeframe","Age","Main Position","Minutes played","Score",] + metrics].rename(columns={'Team within selected timeframe':'Team'})
...
with map_tab:
    if gender == 'Men':
        available_leagues_df = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup.csv')
    if gender == 'Women':
        available_leagues_df = read_csv('https://raw.githubusercontent.com/BG-Kolding/Kolding-Data/main/league_info_lookup_women.csv')
    ...
    st.altair_chart(basemap+map, width='stretch')
    #########
