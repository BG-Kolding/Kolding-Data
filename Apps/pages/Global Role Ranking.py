...
st.set_page_config(page_title="Global Role Ranking", page_icon="🌍")
...
def prep_df(geo_input, region, tiers, time_frame, mins, ages, exp_contracts, expiration_date):
    if gender == 'Men':
...
def prep_similarity_df_filters(geo_input, region, tiers, time_frame):
    ...
    dfs = []
    char_replacements = str.maketrans({" ": "%20", "ü": "u", "ó": "o", "ö": "o", "ã": "a", "ç": "c", "é":"e"})
    for _, row in sim_lg_lookup.iterrows():
        full_league_name = f"{row['League']} {row['Season']}"
        formatted_name = full_league_name.translate(char_replacements)
        if gender == 'Men':
...
def make_rankings(formation, mins, data, role_position_df, leagues, exp_contracts, expiration_date,
                  min_age, max_age, num, normalize_to_100, chosen_team, nationality_chosen,group_all_leagues
                 ):
    ...
    for q in range(len(rank_11)):
        ...
        if group_all_leagues == 'Yes':
            ...
            ranks = dfProspect[['League',"Player", "Main Position", "Team within selected timeframe", "Age", 'Contract expires', 'Minutes played','Passport country',
                                'Distributor Score','Pure Shot-Stopper Score','Balanced Keeper Score','Ball Playing CB Score',
                                'Wide CB Score','Defensive CB Score','Goalscoring CB Score',
                                'Balanced CB Score','Defensive FB Score','Inverted FB Score','Attacking FB Score','Wingback Score','Midfield Anchor Score','Holding Mid Score','Deep-Lying Playmaker Score','Box-to-Box Midfielder Score','Advanced Playmaker Score','Classic 10 Score','Second Striker Score','Traditional Winger Score','Inverted Winger Score','Inside Forward Score','Complete Striker Score','Target Man Score','Playmaking Striker Score','Transitional Threat Score','Pressing Striker Score','Poacher Score',
                               'Wide Playmaker Score']]
            ...
            ranks = ranks[['Player', 'Team','League', 'Age', 'Main Position', 'Score', 'Minutes played','Contract expires','Passport country']]
        ...
        if group_all_leagues == 'No':
            for z in range(len(leagues)):
                dfProspect = df[df.League==leagues[z]]
                dfProspect = df[(df['Minutes played']>=mins)].copy()
                dfProspect = filter_by_position(dfProspect, pos)
                dfProspect = dfProspect.reset_index(drop=True)
                ...
                if normalize_to_100 == 'Yes':
                    for i in range(8,len(ranks.columns)):
                        ranks.iloc[:,i] = NormalizeData(ranks.iloc[:,i])
...
def scout_report(data_frame, gender, league, xtra, template, pos, player_pos, pos_in_sample, mins, minplay, compares, name, ws_name, team, age, sig, extra_text, custom_radar, dist_labels, logo_dict, metric_selections=None):
    plt.clf()
    df = data_frame
...
with st.sidebar:
    ...
    region = st.multiselect(f"{geo_input}(s) to include (leave blank to include all)", lg_lookup[geo_input].unique().tolist(), default=default_region_area)
    tiers = st.multiselect("Tiers to include (leave blank to include all)", ('1','2','3','4','5','6','Youth'))
    time_frame = st.selectbox('Time Frame', ('Current Season','Prior Season','Current & Prior Seasons'))

with st.sidebar:
    st.header('Choose Basic Options')
    group_all_leagues_p = st.selectbox('Group all leagues together for rankings?', ['Yes','No'])
    ...
    if exp_contracts_ == 'Yes':
        exp_contracts = 'y'
        expiration_date = st.date_input("Contract Expires On Or Before", datetime.date(2025, 8, 1), format='YYYY-MM-DD')
    else:
        exp_contracts = 'n'
        expiration_date = '2025-08-01'
    number_of_players = st.slider('Top X Players Per Role  \n(Image limited to 7, table will show all X)', 1, 20, 5)
    normalize_to_100 = st.selectbox('Normalize Scores so #1 = 100?', (['No','Yes']))
...
update_date = lg_lookup[(lg_lookup.League==default_danish_league) & (lg_lookup.Season==default_danish_season)].Date.values[0]


clean_df = prep_df(geo_input, region, tiers, time_frame, mins, ages, exp_contracts, expiration_date)
...
rank_list = make_rankings(formation, mins, clean_df, role_position_df, clean_df.League.unique().tolist(), exp_contracts, expiration_date,
                          min_age=ages[0], max_age=ages[1], num=number_of_players, normalize_to_100=normalize_to_100, chosen_team=chosen_team, nationality_chosen=chosen_nations,group_all_leagues=group_all_leagues_p)
show_ranks = rank_list[['Player','League','Team','Age','Squad Position','Player Pos.','Score','Role Rank','Formation Pos.']].copy()
...
def make_fig(ages,exp_contracts,rank_11_base,show_ranks2,season,lg,normalize_to_100,team_text,chosen_nations):
    ...
    axs['title'].text(0.5, 1.5, team_text,
                     ha='center',va='bottom', size=20, weight='bold', color='#4a2e19')

    axs['title'].text(0.5, 1.35, f'Data via Wyscout | {lg_lookup[(lg_lookup.League==default_danish_league) & (lg_lookup.Season==default_danish_season)].Date.values[0]}',
...
show_ranks = show_ranks[['Player','League','Team','Age','Squad Position','Player Pos.','Score','Role Rank']].copy()

image_tab, table_tab, radar_tab,  notes_tab, map_tab = st.tabs(['Role Ranking Image', 'Role Ranking Table', 'Player Radar Generation', 'Role Score Definitions & Calculations', 'Available Leagues'])

with image_tab:
    fig = make_fig(ages,exp_contracts,rank_11_base,show_ranks2,default_danish_season,default_danish_league,normalize_to_100,team_text,chosen_nations)
    st.pyplot(fig)
...
with radar_tab:
    ...
    with st.form('Player Radar Options'):
        ...
        pteam = st.text_input("Player's Team", "")
        submitted = st.form_submit_button("Submit Options")
        ...
        xtratext = lg_lookup[(lg_lookup.League==default_danish_league) & (lg_lookup.Season==default_danish_season)].Date.values[0]

        try:
            gen = df1[(df1['Player']==player) & (df1['Age']==page) & (df1['Team within selected timeframe']==pteam)]
            plg = gen.League.values[0]
